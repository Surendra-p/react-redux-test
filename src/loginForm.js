...
class LoginTab extends React.Component {
    ...
    handleChangeMail = (event) => {
        this.props.handleChangeEml({ "email": event.target.value })
    }
    handleChangePwd = (event) => {
        this.props.handleChangePwrd({ "password": event.target.value })
    }
    loginSubmit = ()=>{
        let users = {
            user: [{
                ...
                <div className={classes.margin}>
                    <Grid container spacing={8} alignItems="flex-end">
                        ...
                        <Grid item md={true} sm={true} xs={true}>
                            <TextField id="username" label="Username" type="email" fullWidth required onChange={this.handleChangeMail}/>
                        </Grid>
                    </Grid>
                    <Grid container spacing={8} alignItems="flex-end">
                        <Grid item>
...
const mapDispatchToProps = dispatch => ({
    handleChangeEml: (event) => dispatch(handleChangeEmail(event)),
    handleChangePwrd: (event) => dispatch(handleChangePassword(event)),
